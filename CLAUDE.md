...
If Engine later rejects completion with a terminal 409 (`JOB_NOT_CLAIMABLE`, already completed with different output, forbidden worker, unsupported type/provider), recovery marks the local job `failed_non_recoverable` and preserves the artifact for review instead of retrying forever.
